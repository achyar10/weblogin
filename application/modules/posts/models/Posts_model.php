...
class Posts_model extends CI_Model {
    ...
    function get($params = array())
    {
        ...
        {
            $this->db->where('posts_published_date >=', $params['date_start'] . ' 00:00:00');
            $this->db->where('posts_published_date <=', $params['date_end'] . ' 23:59:59');
        }
        ...
        $this->db->select('posts.posts_id, posts_title, posts_description, posts_image,
            ...
            posts_category.category_name, user_user_id, user_full_name, posts_input_date,
            posts_last_update');
        $this->db->join('posts_category', 'posts_category.category_id = posts.posts_category_category_id', 'left');
        $this->db->join('users', 'users.user_id = posts.user_user_id', 'left');
